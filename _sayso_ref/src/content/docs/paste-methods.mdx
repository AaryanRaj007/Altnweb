---
title: "Paste Methods"
description: "How SaySo inserts text, which paste method to use, typing tools for Linux, and clipboard handling."
order: 6
---

After transcription, SaySo pastes the result into whatever text field your cursor is in. How it does this depends on the paste method you have configured in **Settings > Advanced**.
...
On Linux, when using a clipboard-based paste method, SaySo needs a typing tool to simulate the paste keystroke. You can select which tool to use in **Settings > Advanced**. This setting only appears on Linux.

| Tool | Best For |
|------|----------|
| **Auto** (Recommended) | SaySo picks the best available tool automatically |
...
- **Wayland (GNOME)**: See [Handy Discussion #718](https://github.com/cjpais/handy/discussions/718) (upstream project) for GNOME-specific setup

## Clipboard Handling

SaySo has two clipboard handling modes, configurable in **Settings > Advanced**:
...
If SaySo is pasting the wrong text (e.g., your previous clipboard contents instead of the transcription), you may need to increase the paste delay. This is the time SaySo waits between copying text to the clipboard and sending the paste keystroke.
...
Windows prevents non-elevated processes from sending input to elevated windows. If SaySo cannot paste into an application running as administrator, run SaySo as administrator too.
