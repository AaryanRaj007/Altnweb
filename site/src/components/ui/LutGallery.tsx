...
export default function LutGallery({ categories }: LutGalleryProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-10">
      <div className="sticky top-20 z-40 flex items-center justify-end gap-4 p-4 bg-sayso-background/80 backdrop-blur border border-sayso-yellow/20 rounded-xl">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          />
          <span className="text-sayso-text font-bold text-sm">Select Multiple</span>
        </label>
        ...
        {isMultiSelect && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-sayso-yellow text-[#16150f] rounded-lg font-bold text-sm disabled:opacity-50 transition-opacity cursor-pointer"
          >
            {isDownloading ? "Zipping..." : `Download ${selectedLuts.size} LUTs`}
            <Download size={16} />
          </button>
        )}
      </div>
