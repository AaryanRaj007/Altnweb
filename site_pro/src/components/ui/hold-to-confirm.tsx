...
export function HoldToConfirm({
  ...
  useEffect(() => {
    if (phase === "releasing") {
      ...
      setTease(null);
    } else if (phase === "idle" && tease) {
      const timer = setTimeout(() => setTease(null), 1000);
  ...
  return (
    <button
      ...
      className={`relative isolate inline-grid h-10 select-none place-items-center overflow-hidden rounded-full border border-white/10 bg-black/40 px-6 text-sm font-medium text-white/90 backdrop-blur-xl outline-none transition-all duration-300 hover:bg-black/60 hover:border-white/20 focus-visible:ring-2 focus-visible:ring-white/30 ${
    ...
    >
      ...
      <motion.span
        aria-hidden
        style={{ clipPath }}
        className="absolute inset-0 grid place-items-center bg-white px-6 text-black"
      >
        <Faces committed={committed} confirmLabel={confirmLabel}>
          {tease || children}
        </Faces>
